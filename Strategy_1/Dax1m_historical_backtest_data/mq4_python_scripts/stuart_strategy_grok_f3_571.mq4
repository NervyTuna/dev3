...
#property version   "1.04" // Updated version for fixes
#property strict
...
double CalculateRetraction(SessionData &session, double currentPrice) {
   if (currentPrice <= 0) {
      Print("Error: Invalid current price in CalculateRetraction: " + DoubleToString(currentPrice, 2));
      return 0;
   }
   double spreadPoints = GlobalVariableGet(globalVarPrefix + "_Spread");
   double raw = MathMax(session.highPrice - currentPrice, currentPrice - session.lowPrice);
   double retraction = raw / Point;
   Print("Retraction Calc - High: " + DoubleToString(session.highPrice, 2) + 
...
int AdjustEntryLevel(int originalLevel, int retractionLevel) {
   ...
   // If retractionLevel is -1 (cancel) and no open trades, abort session
   if (retractionLevel == -1 && CountOpenTrades() == 0)
      return -1;
   return originalLevel;
}
double CalculateNewEntryPrice(SessionData &session, double sessionOpen, int adjustedLevel, bool isBuy, int retractionLevel) {
   int distancePoints = 0;
   if (adjustedLevel == 1)
      distancePoints = ZoneEntryLevel1;
   else if (adjustedLevel == 2)
      distancePoints = ZoneEntryLevel2;
   else if (adjustedLevel == 3)
      distancePoints = ZoneEntryLevel3;
   else if (adjustedLevel == 4)
      distancePoints = ZoneEntryLevel4;
   double spreadPoints = GlobalVariableGet(globalVarPrefix + "_Spread");
   double adjust = (retractionLevel == 1) ? RetractAdjustBeyondHL : 0;
   if (isBuy)
      return sessionOpen - (distancePoints + adjust + spreadPoints) * Point;
   else
      return sessionOpen + (distancePoints + adjust + spreadPoints) * Point;
}
...
void PlaceTrade(string type, double entryPrice, double lotSize, int sessionNum) {
   ...
   double stopLossPrice = (type == "BUY") 
                         ? entryPrice - StopLossPoints * Point 
                         : entryPrice + StopLossPoints * Point;
...
void CheckAndPlaceTrade(SessionData &session, double currentPrice, int sessionNum) {
   ...
   if (distancePoints < ZoneEntryLevel1) {
      Print("Distance too small: " + DoubleToString(distancePoints, 1));
      return;
   }
   
   int entryLevel = 0;
   if (distancePoints >= ZoneEntryLevel1) entryLevel = 1;
   if (distancePoints >= ZoneEntryLevel2) entryLevel = 2;
   if (distancePoints >= ZoneEntryLevel3) entryLevel = 3;
   if (distancePoints >= ZoneEntryLevel4) entryLevel = 4;
   
   Print("Entry Level: " + IntegerToString(entryLevel));
   if (entryLevel == 0) return;
   
   double retraction = CalculateRetraction(session, currentPrice);
   int retractionLevel = GetRetractionLevel(retraction);
   Print("Retraction: " + DoubleToString(retraction, 1) + ", Retraction Level: " + IntegerToString(retractionLevel));
   ...
   int adjustedLevel = AdjustEntryLevel(entryLevel, retractionLevel);
   // Full recursion to resolve multiple tolerance breaches
   while(true) {
      int nextLevel = AdjustEntryLevel(adjustedLevel, retractionLevel);
      if (nextLevel == -1) return;
      if (nextLevel == adjustedLevel) break;
      adjustedLevel = nextLevel;
   }
   if (adjustedLevel == -1) return;
   
   bool isBuy = (adjustedBid < session.openPrice);
   double entryPrice = CalculateNewEntryPrice(session, session.openPrice, adjustedLevel, isBuy, retractionLevel);
   Print("Trade Setup - Is Buy: " + (isBuy ? "true" : "false") + ", Entry Price: " + DoubleToString(entryPrice, 2));
   
   if (!IsPriceValid(entryPrice)) return;
...
void HandleSweepsAndVolatility() {
   ...
   if ((distanceFromSession1Open >= ZoneCancelLevel) || 
      ...
      CloseAllTrades();
      Print("Sweep Close triggered by 179+ point move.");
   }
